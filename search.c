...
GList* pal_search_get_results(const gchar* search, GDate* date, const gint window)
{
    regex_t preg;
    gint i,j;
    GList* hit_list = NULL;
    ...
    /* put 'date' back where it started */
    if(settings->reverse_order)
	g_date_add_days(date, 1);
    else
	g_date_subtract_days(date, window);
...
int pal_search_view(const gchar* search_string, GDate* date, const gint window, const gboolean number_events)
{
    ...
    pal_output_attr(BRIGHT, _("[ End search results: %s ] [ %d %s found ]\n"),
		    search_string, hit_count, hit_count == 1 ? "event" : "events");
...
PalEvent* pal_search_event_num(gint event_number, GDate** store_date, const gchar* search_string, GDate* date, const gint window)
{
